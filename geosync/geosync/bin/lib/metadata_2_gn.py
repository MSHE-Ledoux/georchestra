...
def publish_2_gn(input, url, login, password, workspace, database_hostname, verbose):
	...
	for line in file_input:
		if "<Esri>" in line:
			print
			"Métadonnée de type ArcGIS à convertir"
	...
	if ('gmd:' in line1) or ('gmd:' in line2):  # que le fichier soit cohérent
		print
		"Fichier avec gmd"  # GMD : Geographic MetaData extensible markup language
		gmd = 'gmd:'
	else:
		gmd = ''
	...
	for element in doc.getElementsByTagName(balise):
		print
		'fileIdentifier en cours de suppression'
	...
	element_descr_txt = doc.createTextNode(
		output.split(".")[0])  # baies_metadata__baies_metadata      #u"baies_metadata__baies_metadataéééééééééé"
	element_descr_gco.appendChild(element_descr_txt)

	print
	b_DigitalTransferOptions
	...
	print
	"suppression de " + titre.text + " " + name_layer_gs
	...
	for rec in csw.records:
		print
		"suppression de " + csw.records[rec].title + csw.records[rec].identifier
...
if __name__ == "__main__":
	...
	parser.add_argument('-u', action="store", dest="url",
	                    default="http://geonetwork-mshe.univ-fcomte.fr:8080/geonetwork/srv/fre/csw-publication")
